...
<div class="row">
    <div class="col-md-6">
        <div class="card">
            <div class="card-header">
                {{ trans('global.stop_account') }}
            </div>

            <div class="card-body">
                <form method="POST" action="{{ route("profile.stop") }}" onsubmit="return prompt('{{ __('global.stop_account_warning') }}') == '{{ auth()->user()->email }}'">
                    @csrf
                    <div class="form-group">
                        <button class="btn  @if(auth()->user()->orders == 0) btn-danger @else btn-success @endif" type="submit">
                            @if(auth()->user()->orders == 0)
                                {{ trans('global.stop') }}
                            @else
                                {{ trans('global.run') }}
                            @endif
