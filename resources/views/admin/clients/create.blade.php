    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.clients.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('eventheads') ? 'is-invalid' : '' }}" name="eventheads[]" id="eventheads" multiple>
                    @foreach($eventheads as $id => $eventhead)
                        <option value="{{ $id }}" {{ in_array($id, old('eventheads', [])) ? 'selected' : '' }}>{{ $eventhead }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('arrived') ? 'is-invalid' : '' }}" type="text" name="arrived" id="arrived" value="{{ old('arrived', '') }}">
