...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\User::TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="type_{{ $key }}" name="type" value="{{ $key }}" {{ old('type', '0') === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('orders') ? 'is-invalid' : '' }}">
                    <input type="hidden" name="orders" value="0">
                    <input class="form-check-input" type="checkbox" name="orders" id="orders" value="1" {{ old('orders', 0) == 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="orders">{{ trans('cruds.user.fields.orders') }}</label>
                </div>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.user.fields.orders_helper') }}</span>
            </div>
            <div class="form-group">
