...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.clientEvent.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.client-events.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ \Carbon::now()->format('Y-m-d') }}" required>
                @if($errors->has('date'))
                    <div class="invalid-feedback">
                        {{ $errors->first('date') }}
                    </div>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ClientEvent::RESPONSIVE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('responsive') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="responsive_{{ $key }}" name="responsive" value="{{ $key }}" {{ old('responsive', '0') === (string) $key ? 'checked' : '' }} required>
